...
class PhysicsUtils {
public:
	_FORCE_INLINE_ static bool ray_cast(Node2D* p_context_2d, const Vector2& p_from, const Vector2& p_to, RayResult& out_result, const Vector<CollisionLayer>& p_collision_layers, const Vector<RID>& p_exclude = {}, bool collide_with_bodies = true, bool collide_with_areas = false, bool hit_from_inside = false) {
		if (p_context_2d) {
			Ref<World2D> world_2d = p_context_2d->get_world_2d();
