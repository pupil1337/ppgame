...
void AsyncLoader::instance(const String& p_path, const Callable& p_callback) {
	...
	loader->add_loader_data(p_path, callable_mp(this, &self_type::_load_callback).bind(p_path));
}

void AsyncLoader::_load_callback(Ref<PackedScene> p_packed_scene, const String& p_path) {
	instancer->add_instancer_data(p_packed_scene, callable_mp(this, &self_type::_instance_callback).bind(p_path));
}

void AsyncLoader::_instance_callback(Node* p_node, const String& p_path) {
	if (path2callbacks.has(p_path)) {
		const Vector<Callable>& callbacks = path2callbacks.get(p_path);
		for (int i = 0; i < callbacks.size(); ++i) {
			callbacks[i].call_deferred(p_node);
		}

		path2callbacks.erase(p_path);
	}
}
